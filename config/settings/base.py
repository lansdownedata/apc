...
TEMPLATES = [
    {
        ...
        "APP_DIRS": True,
        "OPTIONS": {
            "builtins": ["apps.core.templatetags.icons"],
            "context_processors": [
...
# Calendly API v2 — a personal access token generated by the account that owns the
# quick-chat event type (Integrations -> API & Webhooks). Only needed to register and
# inspect webhook subscriptions; the inbound webhook itself needs no token.
CALENDLY_API_TOKEN = env("CALENDLY_API_TOKEN", default="")
