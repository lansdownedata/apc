...
def test_ordered_stops_pickup_mids_dropoff():
    form = BookingRequestForm(_base(
        pickup="123 Main St, Ashburn, VA", pickup_suite="Apt 4", pickup_lat="38.9", pickup_lng="-77.4",
        dropoff="Dulles Intl (IAD)", dropoff_lat="38.95", dropoff_lng="-77.45",
        stops_json=json.dumps([{"address": "Reston Town Center", "suite": "", "lat": 38.95, "lng": -77.35}]),
    ))
    assert form.is_valid(), form.errors
    stops = form.cleaned_data["stops"]
    assert [s["address"] for s in stops] == ["123 Main St, Ashburn, VA", "Reston Town Center", "Dulles Intl (IAD)"]
